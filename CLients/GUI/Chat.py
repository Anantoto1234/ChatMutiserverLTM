...
from tkinter import Button, Toplevel, messagebox, Scrollbar, Canvas
import socket
import json
...
class ChatClient:
    ...
    def create_ui(self):
        ...
        self.message_entry.bind("<Return>", self.send_message)
                # Button frame for emoji
        self.button_frame = tk.Frame(self.root, bg="white")
        self.button_frame.grid(row=2, column=1, sticky="we", padx=10, pady=5)

        self.emoji_button = tk.Button(self.button_frame, text="😀 Gửi Emoji", font=("Arial", 10), command=self.open_emoji_picker)
        self.emoji_button.pack(side="left", padx=5)

    def open_emoji_picker(self):
        """Open a window to select and send an emoji."""
        ...
        emojis = ["😀", "😂", "❤️", "👍", "🎉", "😢", "🔥", "💡", "🎵", "🎂"]
        for emoji in emojis:
            button = Button(emoji_window, text=emoji, font=("Arial", 20), command=lambda e=emoji: self.send_emoji(emoji_window, e))
            button.pack(side="left", padx=5, pady=5)

    def send_emoji(self, emoji_window, emoji):
        """Send the selected emoji to the server."""
        emoji_window.destroy()  # Close the emoji picker window

        if not self.selected_conversation:
            messagebox.showinfo("Thông báo", "Hãy chọn một cuộc trò chuyện để gửi emoji.")
            return

        if self.send_message_to_server(self.selected_conversation.get("id", self.selected_conversation.get("user_id")), emoji):
            self.append_message("You", emoji, datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    ...
    def get_messages_from_server(self, conversation_id, since=None):
        ...
        # Nhận dữ liệu theo luồng
            response = self.receive_full_data(client)
            client.close()
    ...
    def receive_full_data(self, client_socket):
      """Nhận dữ liệu đầy đủ từ server."""
      data = b""
      while True:
           part = client_socket.recv(BUFFER_SIZE)
           data += part
           if len(part) < BUFFER_SIZE:  # Nếu dữ liệu nhận được nhỏ hơn BUFFER_SIZE, đã hết
              break
